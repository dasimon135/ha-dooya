...
from .dooya_protocol import BUTTON_UP, DooyaData
...
class DooyaConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            self._esphome_device = user_input.get(CONF_ESPHOME_DEVICE, "")
        ...
        return self.async_show_form(
            ...
            description_placeholders={"example": "volets-dooya-rf433"},
            errors=errors,
        )
    ...
    async def async_step_learn(
        ...
        if user_input is not None and user_input.get("skip"):
        ...
        # Timeout → proposer saisie manuelle
        return self.async_show_form(
            step_id="learn",
            data_schema=vol.Schema(
                {
